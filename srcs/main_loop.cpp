  ...
  for (size_t i = 0; i < servers.size(); ++i) {
    ...
    if (port_to_fd.count(port)) {
      ...
      try {
        ...
        std::cerr << "Failed to setup server on port " << port << ": "
                  << e.what() << std::endl;
        continue;
      }
    }
    ...
    for (size_t i = 0; i < fds.size(); i++) {
      ...
      if (is_server && (fds[i].revents & POLLIN))
      {
        ...
        if (client_fd >= 0) {
          ...
          std::cout << "New connection: Client " << client_fd
                    << " on Server FD " << fds[i].fd << std::endl;
        }
      } else if (!is_server &&
        ...
        if (bytes <= 0) {
          ...
          if (client->isRequestComplete() &&
            ...
            if (cgi) {
              ...
              std::cout << "CGI started for Client " << fds[i].fd
                        << " on Pipe FD " << cgiFd << std::endl;
            } else {
        ...
        if (client->hasResponseToSend()) {
          int sent = send(fds[i].fd, client->getResponseData(),
        ...
        if (!client->hasResponseToSend() &&
          ...
          if (!client->wantsKeepAlive()) {
            std::cout << "Closing connection for Client " << fds[i].fd
                      << std::endl;
            close(fds[i].fd);
            delete clients[fds[i].fd];
            clients.erase(fds[i].fd);
            ...
            client->clear();
            fds[i].events = POLLIN;
            std::cout << "Response fully sent to Client " << fds[i].fd
                      << " (Keep-Alive)" << std::endl;
          }
        }
      }
