...
ConfigParser::ConfigParser(const std::string& filePath) : _filePath(filePath), _currentTokenIndex(0) {}
...
void ConfigParser::parseServer(ServerConfig& server)
{
    while (_currentTokenIndex < _tokens.size())
    {
        ...
        if (token == "listen")
        {
            ...
            if (nextToken() != ";") throw std::runtime_error("Expected ';' after listen");
        }
        else if (token == "host")
        {
            server.host = nextToken();
            if (nextToken() != ";") throw std::runtime_error("Expected ';' after host");
        }
        else if (token == "server_name")
        {
            server.server_name = nextToken();
            if (nextToken() != ";") throw std::runtime_error("Expected ';' after server_name");
        }
        else if (token == "root")
        {
        ...
        else if (token == "error_page")
        {
            ...
            if (nextToken() != ";") throw std::runtime_error("Expected ';' after error_page");
        }
        else if (token == "client_max_body_size")
        {
            server.client_max_body_size = std::atol(nextToken().c_str());
            if (nextToken() != ";") throw std::runtime_error("Expected ';' after client_max_body_size");
        }
