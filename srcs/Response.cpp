...
/*   Updated: 2026/01/27 18:23:21 by sdavi-al         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Response::Response(Request& request, ServerConfig& serverConfig) 
    : _request(request), _serverConfig(serverConfig), _statusCode(200), _cgiHandler(NULL) {}
...
void Response::generateResponse()
{
    ...
    {
        for (std::map<std::string, std::string>::const_iterator it = location->cgi_extensions.begin(); it != location->cgi_extensions.end(); ++it)
        {
            ...
            {
                if (pos + ext.length() == fullPath.length() || fullPath[pos + ext.length()] == '/')
                {
                    scriptPath = fullPath.substr(0, pos + ext.length());
                    pathInfo = fullPath.substr(pos + ext.length());
    ...
    if (!interpreter.empty())
    {
        ...
        try
        {
            _cgiHandler = new CgiHandler(_request, scriptPath, interpreter);
            _cgiHandler->start(); 
            return;
    ...
    else if (_request.getMethod() == "POST")
    {
        ...
        if (contentType.find("multipart/form-data") != std::string::npos)
        {
            ...
            if (boundaryPos != std::string::npos)
            {
                ...
                std::string extractedBody = _parseMultipartBody(_request.getBody(), boundary);
                if (!extractedBody.empty())
                {
                    bodyToWrite = extractedBody;
                }
            }
        }
...
std::string Response::_parseMultipartBody(const std::string& rawBody, const std::string& boundary)
{
    std::string dashBoundary = "--" + boundary;
    std::string limit = dashBoundary + "--";
    
    size_t startPos = rawBody.find(dashBoundary);
    if (startPos == std::string::npos) return "";
    
    startPos += dashBoundary.length() + 2;
    
    size_t endPos = rawBody.find(dashBoundary, startPos);
    if (endPos == std::string::npos) return "";
    
    size_t headerEnd = rawBody.find("\r\n\r\n", startPos);
    if (headerEnd == std::string::npos || headerEnd > endPos) return "";
    
    size_t contentStart = headerEnd + 4;
    size_t contentLen = endPos - contentStart - 2;
    
    return rawBody.substr(contentStart, contentLen);
}
...
CgiHandler* Response::takeCgiHandler()
{
    CgiHandler* handler = _cgiHandler;
    _cgiHandler = NULL;
    return handler;
}