...
Client::Client(int fd)
    : _fd(fd), _isRequestReady(false), _state(READING_REQUEST),
...
bool Client::wantsKeepAlive() const { return _keepAlive; }
...
bool Client::hasResponseToSend() const { return !_responseBuffer.empty(); }

const char *Client::getResponseData() const { return _responseBuffer.c_str(); }

size_t Client::getResponseSize() const { return _responseBuffer.size(); }
...
Client::ClientState Client::getState() const { return _state; }
...
CgiHandler *Client::getCgiHandler() const { return _cgiHandler; }
...
void Client::handleCgiFinished() {
  if (_responseBuffer.compare(0, 8, "HTTP/1.1") != 0) {
    ...
    if (bodyStart != std::string::npos) {
      ...
      size_t unixBodyStart = _responseBuffer.find("\n\n");
      if (unixBodyStart != std::string::npos) {
        size_t bodySize = _responseBuffer.size() - (unixBodyStart + 2);
        std::stringstream ss;
        ss << "Content-Length: " << bodySize << "\r\n";
        headers += ss.str();
      } else {
        headers +=
            "Content-Length: " + intToString(_responseBuffer.size()) + "\r\n";
      }
    }
